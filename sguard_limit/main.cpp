...
INT WINAPI WinMain(
	...
	if (!status) {
		MessageBox(0,
			"【更新说明】\n\n"
			" 内存补丁 " MEMPATCH_VERSION "：新增限制system占用cpu。\n\n"
			"1. 限制TX游戏启动后“system”进程莫名其妙占用很高cpu（10%甚至以上）。\n\n"
			"2. 修复DNF和LOL弹窗“上报异常”，若出现该问题请先重启电脑再使用。\n\n"
			"【注】LOL本体占用大量CPU是新活动页面没做好导致的，如果官方不修等活动过了就好了。\n\n\n"
